...
void _render_treemap_3(struct TreeNode **data, int data_len,
  ...
  if (bounds.w > bounds.h) {
    ...
    for (int n=1; n<=data_len; n++) {
      ...
      for(int j=0;j<n;j++){
        average_normalized_ratio += aspect(bounds.w*slice_sum/data_sum, bounds.h*data[j]->data/slice_sum);
      }
    ...
    for(int i=0;i<best_n;i++){
      ...
      char label[100];
      sprintf(label, "%d", (int)data[i]->data);
      char tooltip[100];
      sprintf(tooltip, "%s %d", data[i]->label, (int)data[i]->data);
    ...
    for (int n=1; n<=data_len; n++) {
      ...
      float average_normalized_ratio = 0;
      for(int j=0;j<n;j++){
        average_normalized_ratio += aspect(bounds.h*slice_sum/data_sum, bounds.w*data[j]->data/slice_sum);
      }
      average_normalized_ratio /= n;
...
void render_treemap_3(struct TreeNode *data,
  ...
  qsort(data->children, data->num_children, sizeof(struct TreeNode *), compare_tree_desc);
  _render_treemap_3(data->children, data->num_children, render_func, (struct Rect){0, 0, 1, 1});
